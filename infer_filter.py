...
def inference(args: argparse.Namespace) -> Optional[torch.Tensor]:
    ...
    Raises:
        NotImplementedError: If the reference dataset is not supported.
    ...
    if args.wandb:
        ...
        if wandb_id_file_path.exists():
            ...
            wandb.init(project="temporal_scale", name=args.name, resume=resume_id, config=args) # type: ignore
        else:
    ...
    method = args.method # This is the method used to generate the explanations
    num_top_samples = args.p # This is the number of top samples to be selected
    ...
    if not os.path.exists(args.max_path):
        ...
        torch.save(max_activations, args.max_path) # type: ignore
    ...
    for f in tqdm(args.f): # We are looping over the filters ? Added by improved_lavise
        ...
        with torch.no_grad():
            ...
            filter_dataset = torch.utils.data.Subset(dataset, sorted_samples[f, :num_top_samples]) # type: ignore
            filter_dataloader = torch.utils.data.DataLoader(filter_dataset, batch_size=1, # type: ignore
            ...
            for i, batch in enumerate(filter_dataloader):
                ...
                predict = explain(method, model, data_, activation, c, xf, threshold) # type: ignore
